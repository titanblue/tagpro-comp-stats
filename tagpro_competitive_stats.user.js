...
document.onreadystatechange = () => {
    if (document.readyState === "complete"){
        setTimeout(function(){
            if(!location.port || location.port < 8000){ // add link to automatically create a competitive group
...
(function(window) {
    ...
    if(!window.tagpro && window.location.port >=8000) {//comp game is detected when the tagpro object does not exist
        ...
        function goodCap(){
            ...
            capUpdate(updateRedCaps,updateBlueCaps,startTime,groupPort,tableExport,teamNum);
        }
        function badCap(){
            if (userTeam == 1){
                updateBlueCaps += 1;
        ...
        GM_setValue("backscorePlayer",GM_getValue("backscorePlayer","Some%20Ball")+"S");
        ...
        tagpro.ready(function(){
            ...
            function setStartTime (){
                ...
                capUpdate(tagpro.score.r, tagpro.score.b, specStartTime, specGroupPort, capStats[0], capStats[1]); // first update should come from here since it has the correct start time
            }
            ...
            tagpro.socket.on("score", function(data){ // Cap update condition
                if (firstUpdate === true && (tagpro.score.r != 0 || tagpro.score.b != 0)){
                    ...
                    var currentId = sortByScore(Object.getOwnPropertyNames(tagpro.players));
                    var capStats = getSpecStats(currentId);
                    capUpdate(specRedCaps,specBlueCaps,specStartTime,specGroupPort,capStats[0],capStats[1]);
                }
                else {
                    firstUpdate = true;
                }
            ...
            tagpro.socket.on("end", function (data) { //  submit stats when the game ends
                ...
                submitStats(specRedCaps,specBlueCaps,specStats[0],specStats[1],specStartTime,specGroupPort,true);
                endSubmit = true;
            });
...
// Misc Functions
function capUpdate(updateRedCaps,updateBlueCaps,startTime,groupPort,tableExport,teamNum){ // send cap update
    var y = new Date();
    ...
    tagpro.ready(function(){
        ...
        socket.on("play", function(){
            ...
            if (tagpro.group.players[GM_getValue("tpUserId",undefined)]){
                ...
                GM_setValue("backscorePlayer",encodeURIComponent("Some%20Ball%20%2B"+GM_info.script.version+"%24"));
            }
